...
Use when the partner attaches this skill and provides the **sketch plan** path: the separate `.plan.md` from **step 4 of [solutionize](../solutionize/SKILL.md)**. That plan includes YAML **`todos`** (one per phase, each **title** matches the corresponding **`### Phase N - …`** heading in **`plan`**) plus **`## 4 - Sketch`** in the body.
